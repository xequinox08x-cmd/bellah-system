...
function buildFacebookMessage(content: string, hashtags: string | null) {
...
export async function publishSystemContent(aiContentId: number): Promise<PublishSystemContentResult> {
  ...
  try {
    ...
    if (content.facebook_post_id) {
      ...
      const publishResponse = await graphPost<FacebookPublishResponse>(`${config.pageId}/feed`, {
        message: buildFacebookMessage(content.content, content.hashtags),
      });
      facebookPostId = normalizeOptionalString(publishResponse.id);
  ...
  return toPublishResult(updated, initialMetricsSynced);
}
