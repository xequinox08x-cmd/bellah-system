...
function QuickActionCard({
  ...
  return (
    <button
      onClick={onClick}
      className="cursor-pointer group relative w-full text-left rounded-xl border border-[#E5E7EB] bg-white p-5 hover:border-[#F9A8C0] hover:shadow-md transition-all duration-200 active:scale-[0.98] overflow-hidden"
    >
      ...
      <p className="text-[#111827] text-sm" style={{ fontWeight: 600 }}>
        {label}
      </p>
  ...
  return (
    <div className="bg-white rounded-xl border border-[#E5E7EB] px-5 py-4 flex items-center gap-4">
      ...
      <div className="min-w-0">
        <p className="text-lg text-[#111827]" style={{ fontWeight: 700 }}>
          {value}
        </p>
...
export default function StaffDashboard() {
  ...
  const myDrafts = useMemo(
    () =>
      contentItems
        .filter((c) => c.createdByRole === 'staff' && ['draft', 'pending', 'rejected'].includes(c.status))
        .slice(0, 6),
    [contentItems],
  )
  ...
  return (
    <div className="space-y-5 pb-6">
      ...
      <div className="flex items-start justify-between">
        ...
        {/* Subtle role badge */}
        <span className="flex items-center gap-1.5 text-[20px] px-2.5 py-1 bg-[#FCE7F3] text-[#EC4899] rounded-full border border-[#F9A8C0]/40 mt-1">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-3 gap-3">
        ...
        <MiniStat
          ...
          iconColor="text-blue-600"
          note={
            rejectedCount > 0 ? `${rejectedCount} rejected` : pendingCount > 0 ? `${pendingCount} pending` : 'All clear'
          }
        />
        <MiniStat
                  ...
                  <tbody>
                    {todaySales.items.map((s, i) => (
                      ...
                      >
                        ...
                        <td className="px-4 py-3 text-xs text-[#6B7280] truncate max-w-[100px]">{s.customerName}</td>
                        <td className="px-4 py-3 text-xs text-right text-[#374151]">×{s.qty}</td>
                        <td className="px-4 py-3 text-right">
                          <p className="text-xs text-[#111827]" style={{ fontWeight: 600 }}>
                            ₱{s.lineTotal.toFixed(2)}
                          </p>
                          <p className="text-[10px] text-emerald-500">+₱{s.lineProfit.toFixed(2)}</p>
                        </td>
                      </tr>
                    ))}
                  </tbody>
                ...
                {myDrafts.map((c) => {
                  ...
                  return (
                    <div key={c.id} className="px-5 py-3.5">
                      ...
                      {/* Rejection reason */}
                      {c.rejectionReason && (
                        <p className="text-[10px] text-red-400 mt-1 line-clamp-2">"{c.rejectionReason}"</p>
                      )}
                      {/* Product tag */}
            ...
            {upcomingPosts.map((c) => (
              <div key={c.id} className="px-5 py-4 flex flex-col gap-2">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <span
                    className={`text-[10px] px-2 py-0.5 rounded-full capitalize ${STATUS_STYLES[c.status]?.pill ?? ''}`}
                  >
                    {c.status}
                  </span>
                </div>
