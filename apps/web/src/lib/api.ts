...
export const api = {
  ...
  async getDashboardSalesRecords() {
    const res = await fetch(`${API_BASE}/dashboard/sales-records`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || 'Failed to load dashboard sales records');
    return data as {
      ok: boolean;
      data: Array<{
        id: string;
  ...
  async getStaffTodaySales() {
    const res = await fetch(`${API_ROOT}/api/staff/dashboard/today-sales`);
    const data = await res.json();
    if (!res.ok) throw new Error(data.message || "Failed to load today's sales");
    return data as {
      ok: boolean;
      todaySales: {
